  ...
  return (
    <div className="min-h-screen bg-black text-white px-10 py-8">
      ...
      <div className="flex justify-center items-center h-[70vh] w-full transition-all duration-500 ease-in-out">
        {selectedService ? (
          <Link to="/works"> {/* Link appears only if an image exists */}
            <div
...
export default function DesignAgency() {
  ...
  return (
    <div className="min-h-screen bg-black text-white px-10 py-8 relative">
      <CustomCursor hovered={cursorHover} />
      ...
      <div className="flex justify-center items-center h-[70vh] w-full transition-all duration-500 ease-in-out">
        {selectedService ? (
          ...
          <div className="w-76 h-76 border border-white rounded-full flex items-center justify-center relative overflow-hidden transition-all duration-500">
            <h1 className="text-5xl font-bold text-center text-white font-[Orbitron]">
              Services
            </h1>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 transition-all">
        {services.map((service, index) => (
          <div
            key={index}
            className={`p-6 border border-gray-600 rounded-3xl shadow-2xl cursor-pointer transition-all text-white w-full max-w-md mx-auto text-left ${
              selectedService?.title === service.title ? "scale-105 border-white shadow-lg" : "opacity-100"
            }`}
            onClick={() => setSelectedService(service)}
